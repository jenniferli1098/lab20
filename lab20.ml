...
value are black *)
let threshold (threshold : float) =
  List.map  (fun row -> List.map (fun v -> if v <= threshold then 0. else 1.) row)
...
let depict img =
  ...
  let depict_pix v r c =
    let lvl = int_of_float (255. *. (1. -. v)) in Graphics.set_color (Graphics.rgb lvl lvl lvl);
    plot c (y - r) in
...
let dither =
  List.map (fun row -> row |> List.map (fun v -> if v > Random.float 1. then 1. else 0.))
    
let mona = Monalisa.image ;;
  
  depict mona ;;
  
  let mona_threshold = threshold 0.75 mona ;;
